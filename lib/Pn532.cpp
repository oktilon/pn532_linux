...
#ifdef PN532DEBUG
        log_dbg(F("Firmware doesn't match!"));
#endif
        return 0;
    }
...
bool Pn532::sendCommandCheckAck(uint8_t* cmd, uint8_t cmdlen,
    ...
    if (!readack()) {
#ifdef PN532DEBUG
        log_dbg(F("No ACK frame received!"));
#endif
        return false;
    }
...
#ifdef PN532DEBUG
    log_dbg("Writing P3 GPIO: %02hhX", pn532_packetbuffer[1]);
#endif
...
uint8_t Pn532::readGPIO(void) {
    ...
    switch (pn532_packetbuffer[p3offset + 2]) {
        case 0x00:  // Using UART
            log_dbg(F("Using UART (IO = 0x00)"));
            break;
        case 0x01:  // Using I2C
            log_dbg(F("Using I2C (IO = 0x01)"));
            break;
        case 0x02:  // Using SPI
            log_dbg(F("Using SPI (IO = 0x02)"));
            break;
    }
#endif
...
#ifdef MIFAREDEBUG
    log_dbg("Setting MxRtyPassiveActivation to %hhu", maxRetries);
#endif
...
#ifdef MIFAREDEBUG
    log_dbg("Found %hhu tags", pn532_packetbuffer[7]);
#endif
...
#ifdef MIFAREDEBUG
    log_dbg("ATQA: 0x%hX, SAK: 0x%hhX", sens_res, pn532_packetbuffer[11]);
#endif
...
bool Pn532::inDataExchange(uint8_t* send, uint8_t sendLength,
...
#ifdef PN532DEBUG
        log_dbg(F("APDU length too long for packet buffer"));
#endif
        return false;
    }
    uint8_t i;
    ...
    if (!waitready(1000)) {
#ifdef PN532DEBUG
        log_dbg(F("Response never received for APDU..."));
#endif
        return false;
    }
    ...
    if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 &&
...
#ifdef PN532DEBUG
            log_dbg("Length (%hhu) check (%hhu) invalid", length, (~length) + 1);
#endif
            return false;
        }
        if (pn532_packetbuffer[5] == PN532_PN532TOHOST &&
...
#ifdef PN532DEBUG
                log_dbg(F("Status code indicates an error"));
#endif
                return false;
            }
            ...
            *responseLength = length;
            ...
            log_dbg("Don't know how to handle this command: %hhu", pn532_packetbuffer[6]);
            return false;
        }
    } else {
        log_dbg(F("Preamble missing"));
        return false;
    }
}
...
bool Pn532::inListPassiveTarget() {
...
#ifdef PN532DEBUG
    log_dbg(F("About to inList passive target"));
#endif

    if (!sendCommandCheckAck(pn532_packetbuffer, 3, 1000)) {
#ifdef PN532DEBUG
        log_dbg(F("Could not send inlist message"));
#endif
        return false;
    }
    ...
    if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 &&
        ...
        if (pn532_packetbuffer[5] == PN532_PN532TOHOST &&
            ...
            if (pn532_packetbuffer[7] != 1) {
#ifdef PN532DEBUG
                log_dbg(F("Unhandled number of targets inlisted"));
#endif
                log_dbg("Number of tags inlisted: %hhu", pn532_packetbuffer[7]);
                return false;
            }

            _inListedTag = pn532_packetbuffer[8];
            log_dbg("Tag number: %hhd", _inListedTag);

            return true;
        } else {
#ifdef PN532DEBUG
            log_dbg(F("Unexpected response to inlist passive host"));
#endif
            return false;
        }
    } else {
...
uint8_t Pn532::mifareclassic_ReadDataBlock(uint8_t blockNumber,
                                           uint8_t* data) {
#ifdef MIFAREDEBUG
    log_dbg("Trying to read 16 bytes from block %hhu", blockNumber);
#endif
    ...
    if (!sendCommandCheckAck(pn532_packetbuffer, 4)) {
#ifdef MIFAREDEBUG
        log_dbg(F("Failed to receive ACK for read command"));
#endif
        return 0;
    }
...
uint8_t Pn532::mifareclassic_WriteDataBlock(uint8_t blockNumber,
                                            uint8_t* data) {
#ifdef MIFAREDEBUG
    log_dbg("Trying to write 16 bytes to block %hhu", blockNumber);
#endif
...
#ifdef MIFAREDEBUG
        log_dbg(F("Failed to receive ACK for write command"));
#endif
        return 0;
    }
    delay(10);
...
uint8_t Pn532::mifareultralight_ReadPage(uint8_t page,
    ...
    if (page >= 64) {
#ifdef MIFAREDEBUG
        log_dbg(F("Page value out of range"));
#endif
        return 0;
    }

#ifdef MIFAREDEBUG
    log_dbg("Reading page %hhu", page);
#endif
...
#ifdef MIFAREDEBUG
    log_dbg("Trying to write 4 byte page %hhu", page);
#endif
